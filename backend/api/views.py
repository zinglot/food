...
class UserSubscriptionsViewSet(mixins.ListModelMixin,
    ...
    """Getting a list of all user subscriptions."""
...
class IngredientViewSet(viewsets.ReadOnlyModelViewSet):
    """Getting information about the ingredients."""
...
class RecipeViewSet(viewsets.ModelViewSet):
    """Create/edit/delete a recipe.
       Getting information about recipes.
       Add recipes to favorites and shopping list.
       Sending a file with a list of recipes.
    """
    ...
    def favorite(self, request, pk):
        """Deleting/adding recipes to favorites."""
        recipe = get_object_or_404(Recipe, id=pk)
        if request.method == 'POST':
            return create_model_instance(request, recipe, FavoriteSerializer)

        if request.method == 'DELETE':
            error_message = 'Этого рецепта нет в вашем избранном.'
    ...
    def shopping_cart(self, request, pk):
        """Deleting/adding to the shopping list."""
        recipe = get_object_or_404(Recipe, id=pk)
        if request.method == 'POST':
    ...
    def download_shopping_cart(self, request):
        """Sending a file with a shopping list."""
